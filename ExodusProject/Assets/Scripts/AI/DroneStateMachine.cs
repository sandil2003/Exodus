...
public class DroneStateMachine : MonoBehaviour
{
    ...
    void RequestNextPatrolPath()
    {
        ...
        PathfindingCoordinator.Instance.RequestPath(droneId, droneNode, targetNode);
    }
    ...
    public void RequestInterceptPath()
    {
        ...
        PathfindingCoordinator.Instance.RequestPath(droneId, droneNode, lastKnownPlayerNode);
    }
    ...
    void OnPathFailed(int id)
    {
        ...
        Debug.Log($"[DroneStateMachine] Drone {droneId}: A* path failed — switching to SEARCH.");

        if (CurrentState == DroneState.ALERT)
            TransitionTo(DroneState.SEARCH);
    }
