...
public class AStarPathfinder : MonoBehaviour
{
    ...
    // Debug Data
    public List<int> LastOpenSetNodes { get; private set; } = new List<int>();
    public HashSet<int> LastClosedSetNodes { get; private set; } = new HashSet<int>();
    ...
    private bool ShouldStoreDebugData()
    {
        return DebugVisualizer.Instance != null && DebugVisualizer.Instance.showDebug;
    }
    ...
    public List<Vector3> FindPath(int startNodeId, int goalNodeId, NavMeshGraph graph)
    {
        ...
        // Reset debug data only if needed
        if (ShouldStoreDebugData())
        {
            LastOpenSetNodes.Clear();
            LastClosedSetNodes.Clear();
        }
        ...
        if (ShouldStoreDebugData()) LastOpenSetNodes.Add(startNodeId);
        ...
        while (openSet.Count > 0)
        {
            ...
            if (ShouldStoreDebugData())
            {
                LastOpenSetNodes.Remove(current);
                LastClosedSetNodes.Add(current);
            }
            ...
            foreach (GraphEdge edge in graph.GetNeighbours(current))
            {
                ...
                if (!gScore.ContainsKey(edge.toNodeId) || tentativeGScore < gScore[edge.toNodeId])
                {
                    ...
                    if (ShouldStoreDebugData() && !LastOpenSetNodes.Contains(edge.toNodeId))
                        LastOpenSetNodes.Add(edge.toNodeId);
                }
            }
        }
